...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		int send_size = sendto(cln_sock, buf, strlen(buf), 0, (struct sockaddr*)&remote_sockaddr, sizeof(remote_sockaddr));
		if(send_size<0) {
			err_num = WSAGetLastError();
			printf("sendto err: %d\n", err_num);
			break;
		...
		int recv_size = recvfrom(cln_sock, buf, sizeof(buf), 0, (struct sockaddr*)&sockaddr, &sockaddr_size);
		if(recv_size<0) {
			err_num = WSAGetLastError();
